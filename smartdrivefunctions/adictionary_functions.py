...
import agdrive_functions as agf
import json
import aiohttp
import asyncio
import time
...
def generate_folder_dictionary(structure_dict, information_dict,folder_dict=None):
    ...
    def recursive_folder_processing(current_dict,key=""):
        ...
        if folder_id:
            # Extract metadata from information_dict using folder_id
            #folder_meta = information_dict.get(folder_id, {})
            # folder_dict[folder_id] = {
            #     **folder_meta,
            #     "contained_file_ids": contained_file_ids,
            #     "contained_folder_ids": contained_folder_ids,
            #     "title": base_key,
...
async def main(creds, processes_to_run):
    ...
    if "structure" in processes_to_run:
        ...
        with open('astructure_dictionary.yaml', 'w') as outfile:
    ...
    if "information" in processes_to_run:
        ...
        with open("astructure_dictionary.yaml", 'r') as file:
            structure_dictionary = yaml.load(file, Loader=yaml.FullLoader)
        if os.path.exists("ainformation_dictionary.yaml"):
            with open("ainformation_dictionary.yaml", 'r') as file:
        ...
        with open('ainformation_dictionary.yaml', 'w') as outfile:
